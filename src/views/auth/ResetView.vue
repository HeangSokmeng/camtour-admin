        ...
        <div class="auth-form-box">
            ...
            <form class="mt-5"
                ...
                <div class="position-relative mb-2">
                    <input class="form-control form-icon-input pe-6 cdl-pass"
                           id="password"
                    ...
                    <div class="invalid-feedback"
                         v-if="vv.new_pass.$error">
                        {{ vv.new_pass.$errors[0].$message }}
                    </div>
                    <button type="button"
                            class="btn px-3 py-0 position-absolute end-0 fs-7 text-body-tertiary"
                            style="top: 7px;">
                        <span v-show="!state.is_show_pass"
                              class="uil uil-eye"
                              @click="state.is_show_pass = true"></span>
                        <span v-show="state.is_show_pass"
                              class="uil uil-eye-slash"
                              @click="state.is_show_pass = false"></span>
                    </button>
                </div>
                <div class="position-relative mb-4">
                    ...
                    <div class="invalid-feedback"
                         v-if="vv.new_pass_confirmation.$error">
                        {{ vv.new_pass_confirmation.$errors[0].$message }}
                    </div>
                    <button type="button"
                            class="btn px-3 py-0 position-absolute end-0 fs-7 text-body-tertiary"
                            style="top: 7px;">
                        <span v-show="!state.is_show_confirm"
                              class="uil uil-eye"
                              @click="state.is_show_confirm = true"></span>
                        <span v-show="state.is_show_confirm"
                              class="uil uil-eye-slash"
                              @click="state.is_show_confirm = false"></span>
                    </button>
                </div>
                <button class="btn btn-primary w-100"
                        :disabled="state.ldi_reset"
                        type="submit">
