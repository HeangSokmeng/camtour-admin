    ...
    <div class="col mx-auto">
        <div class="auth-form-box">
            <div class="text-center">
                ...
                <p class="text-body-tertiary mb-4">An email containing a 6-digit verification code has been sent to the
                    email address - {{ obscureEmail(state.forgot_email) }} </p>
                <form class="verification-form"
                      @submit.prevent="onSubmitOTP()">
                    <div class="d-flex align-items-center gap-2 mb-4">
                        <input class="form-control px-2 text-center text-uppercase"
                               type="text"
                               ref="one"
                               maxlength="1"
                        ...
                        <input class="form-control px-2 text-center text-uppercase"
                               ...
                               @keyup="onThreeInput($event)"/><span>-</span>
                        <input class="form-control px-2 text-center text-uppercase"
                               type="text"
                               ref="four"
                               maxlength="1"
                    ...
                    <p class="fs-9 mb-0">Didn’t receive the code? <a role="button"
                                                                     class="text-primary"
                                                                     :disabled="state.ldi_resend"
                                                                     @click="onResetOTP()">{{
                            state.ldi_resend ? 'Sending...' : 'Click to resend'
                        }}</a></p>
                </form>
            </div>
        </div>
