                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="(category, index) in filteredCategories" :key="category.id">
                            ...
                            <td class="align-middle">
                                <img :src="category.image" alt="Category Image" class="img-thumbnail"
                            ...
                            <td class="align-middle text-end">
                                <button class="btn btn-sm btn-primary me-2" @click="editCategory(category.id)">
        ...
        <div class="modal-content">
            ...
            <form class="row g-3 needs-validation" novalidate @submit.prevent="handleSubmit">
                <div class="col-md-6">
                    ...
                    <input v-model="newCategory.name" class="form-control" id="categoryName" type="text" required />
                    <div class="invalid-feedback">Category name is required</div>
                </div>
                <div class="col-md-6">
                    <label class="form-label" for="categoryImage">Image</label>
                    <input class="form-control" id="categoryImage" type="file" @change="handleImageUpload" />
                </div>
                <div class="col-md-12">
                    <label class="form-label" for="categoryDescription">Description</label>
                    <input v-model="newCategory.description" class="form-control" id="categoryDescription"
                        type="text" />
                </div>
...
const fetchCategories = async () => {
    ...
    try {
        ...
        if (res.data.result && Array.isArray(res.data.data)) {
            ...
            state.error = res.data.message || "Failed to fetch categories";
        }
    } catch (error) {
...
const deleteCategory = async (categoryId) => {
    ...
    try {
        const res = await axios.delete(`/api/categories/${categoryId}`,  globalStore.getAxiosHeader());
        if (res.data.result) {
